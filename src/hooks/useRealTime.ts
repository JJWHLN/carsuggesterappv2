...
import {
  ...
  StateUpdate,
  StateConflict,
} from '../services/realtime/RealTimeStateManager';
...
export function useLivePrice(carId: string): {
  ...
  const [priceHistory, setPriceHistory] = useRealTimeState<
    Array<{ price: number; timestamp: number }>
  >(`prices.${carId}.history`, []);
  const [isTracking, setIsTracking] = useRealTimeState<boolean>(
    `prices.${carId}.tracking`,
    false,
  );
...
export function useLiveChat(chatId: string): {
  ...
  const [messages, setMessages] = useRealTimeState<any[]>(
    `chats.${chatId}.messages`,
    [],
  );
  const [typingUsers, setTypingUsers] = useRealTimeState<string[]>(
    `chats.${chatId}.typing`,
    [],
  );
  const [isConnected, setIsConnected] = useRealTimeState<boolean>(
    `chats.${chatId}.connected`,
    false,
  );
  const stateManager = getRealTimeStateManager();
...
export function useLiveInventory(filters?: {
  ...
  const [cars, setCars] = useRealTimeState<any[]>(
    `inventory.${filterKey}.cars`,
    [],
  );
  const [totalCount, setTotalCount] = useRealTimeState<number>(
    `inventory.${filterKey}.total`,
    0,
  );
  const [newArrivals, setNewArrivals] = useRealTimeState<any[]>(
    `inventory.new_arrivals`,
    [],
  );
  const [priceDrops, setPriceDrops] = useRealTimeState<any[]>(
    `inventory.price_drops`,
    [],
  );
...
export function useBatchStateUpdate(): (
...
export function useRealTimeConnection(): {
  ...
  const [isReconnecting, setIsReconnecting] = useRealTimeState<boolean>(
    'connection.reconnecting',
    false,
  );
  const [lastConnected, setLastConnected] = useRealTimeState<number | null>(
    'connection.last_connected',
    null,
  );
  const [pendingUpdates, setPendingUpdates] = useRealTimeState<number>(
    'connection.pending_updates',
    0,
  );
...
export function useLiveMarketData(
  ...
  const [trends, setTrends] = useRealTimeState<any[]>(
    `market.${filterKey}.trends`,
    [],
  );
  const [insights, setInsights] = useRealTimeState<any[]>(
    `market.${filterKey}.insights`,
    [],
  );
  const [alerts, setAlerts] = useRealTimeState<any[]>(
    `market.${filterKey}.alerts`,
    [],
  );
  const [averagePrice, setAveragePrice] = useRealTimeState<number | null>(
    `market.${filterKey}.avg_price`,
    null,
  );
  const [demandLevel, setDemandLevel] = useRealTimeState<
    'low' | 'medium' | 'high' | null
  >(`market.${filterKey}.demand`, null);
...
export function usePersistentRealTimeState<T = any>(
  ...
  const [state, setState, { loading, error }] = useRealTimeState<T>(
    path,
    defaultValue,
  );
