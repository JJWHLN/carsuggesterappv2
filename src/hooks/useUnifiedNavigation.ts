...
export function useUnifiedNavigation() {
  ...
  const triggerHaptics = useCallback(
    async (type: 'light' | 'medium' | 'heavy' = 'light') => {
      try {
        switch (type) {
          case 'light':
  ...
  const navigateToCarDetails = useCallback(
    (carId: string | number, options?: NavigationOptions) => {
      navigateTo(`/car/${carId}`, {
        analyticsEvent: 'view_car_details',
        ...options,
      });
    },
    [navigateTo],
  ...
  const navigateToModelDetails = useCallback(
    (modelId: string | number, options?: NavigationOptions) => {
      navigateTo(`/model/${modelId}`, {
        analyticsEvent: 'view_model_details',
        ...options,
      });
    },
    [navigateTo],
  ...
  const navigateToBrandDetails = useCallback(
    (brandId: string | number, options?: NavigationOptions) => {
      navigateTo(`/brand/${brandId}`, {
        analyticsEvent: 'view_brand_details',
        ...options,
      });
    },
    [navigateTo],
  ...
  const navigateToReviewDetails = useCallback(
    (reviewId: string | number, options?: NavigationOptions) => {
      navigateTo(`/review/${reviewId}`, {
        analyticsEvent: 'view_review_details',
        ...options,
      });
    },
    [navigateTo],
  ...
  const navigateToMarketplace = useCallback(
    (options?: NavigationOptions) => {
      navigateTo('/marketplace', {
        analyticsEvent: 'open_marketplace',
        ...options,
      });
    },
    [navigateTo],
  ...
  const navigateToAuth = useCallback(
    (
      screen: 'sign-in' | 'sign-up' | 'forgot-password' = 'sign-in',
      options?: NavigationOptions,
    ) => {
      navigateTo(`/auth/${screen}`, {
        analyticsEvent: `open_${screen}`,
        ...options,
      });
    },
    [navigateTo],
  ...
  const navigateToMarketplaceWithFilters = useCallback(
    (filters: Record<string, any>, options?: NavigationOptions) => {
...
export function useCommonScreenActions() {
  ...
  const handleShare = useCallback(
    async (title: string, url?: string, message?: string) => {
      await triggerHaptics('light');
...
export function useCommonListActions() {
  ...
  const handleRefresh = useCallback(
    async (refreshFunction: () => void | Promise<void>) => {
      await triggerHaptics('light');
  ...
  const handleLoadMore = useCallback(
    async (loadMoreFunction: () => void | Promise<void>) => {
