...
export function useUnifiedDataFetching<T>(
  ...
  const loadData = useCallback(
    async (reset = false, searchTerm?: string, pageNum?: number) => {
      ...
      try {
        ...
        const query =
          searchTerm ?? (enableSearch ? debouncedSearchQuery : undefined);
        ...
        if (enablePagination || enableInfiniteScroll) {
          const newData = Array.isArray(processedResult)
            ? processedResult
            : [processedResult];
          ...
          if (reset) {
            ...
            setData((prev) => {
              ...
              const existingIds = new Set(
                prevArray.map((item: any) => item.id).filter(Boolean),
              );
