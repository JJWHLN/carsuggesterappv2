...
export function transformDatabaseVehicleListingToCar(
  dbListing: DatabaseVehicleListing,
): Car {
  if (!dbListing || typeof dbListing !== 'object') {
    ...
    return {
      /* default/empty Car structure */
    } as Car;
  }

  const location = (() => {
    const city = isValidString(dbListing.location_city)
      ? dbListing.location_city.trim()
      : '';
    const state = isValidString(dbListing.location_state)
      ? dbListing.location_state.trim()
      : '';
  ...
  return {
    ...
    year:
      isValidNumber(dbListing.year) &&
      dbListing.year >= 1900 &&
      dbListing.year <= new Date().getFullYear() + 2
        ? dbListing.year
        : new Date().getFullYear(),
    price:
      isValidNumber(dbListing.price) && dbListing.price >= 0
        ? dbListing.price
        : 0,
    mileage:
      isValidNumber(dbListing.mileage) && dbListing.mileage >= 0
        ? dbListing.mileage
        : 0,
    location,
    images,
    created_at: isValidDate(dbListing.created_at)
      ? dbListing.created_at
      : new Date().toISOString(),
    title: isValidString(dbListing.title)
      ? dbListing.title.trim()
      : `${dbListing.year || ''} ${dbListing.make || ''} ${dbListing.model || ''}`.trim(),
    description: isValidString(dbListing.description)
      ? dbListing.description.trim()
      : undefined,
    features,
    dealer,
    condition: isValidString(dbListing.condition)
      ? dbListing.condition
      : undefined,
    fuel_type: isValidString(dbListing.fuel_type)
      ? dbListing.fuel_type
      : undefined,
    transmission: isValidString(dbListing.transmission)
      ? dbListing.transmission
      : undefined,
    exterior_color: isValidString(dbListing.exterior_color)
      ? dbListing.exterior_color.trim()
      : undefined,
    interior_color: isValidString(dbListing.interior_color)
      ? dbListing.interior_color.trim()
      : undefined,
  };
}

export function transformDatabaseReviewToReview(
  ...
  if (!dbReview || typeof dbReview !== 'object') {
    ...
    return {
      /* default/empty Review structure */
    } as Review;
  }

  const carMake =
    dbReview.car_models?.brands?.name &&
    isValidString(dbReview.car_models.brands.name)
      ? dbReview.car_models.brands.name.trim()
      : 'Unknown Make';
  const carModel =
    dbReview.car_models?.name && isValidString(dbReview.car_models.name)
      ? dbReview.car_models.name.trim()
      : 'Unknown Model';
  const carYear =
    isValidNumber(dbReview.car_models?.year) &&
  ...
  const sections = {
    summary: isValidString(dbReview.summary)
      ? dbReview.summary.trim()
      : undefined, // Keep summary distinct
    performance: isValidString(dbReview.performance)
      ? dbReview.performance.trim()
      : undefined,
    exterior: isValidString(dbReview.exterior)
      ? dbReview.exterior.trim()
      : undefined,
    interior: isValidString(dbReview.interior)
      ? dbReview.interior.trim()
      : undefined,
    practicality: isValidString(dbReview.practicality)
      ? dbReview.practicality.trim()
      : undefined,
    tech: isValidString(dbReview.tech) ? dbReview.tech.trim() : undefined,
    verdict: isValidString(dbReview.verdict)
      ? dbReview.verdict.trim()
      : undefined,
  };
  ...
  const title = isValidString(dbReview.summary) // Prefer summary for title if available and short enough
    ? dbReview.summary.substring(0, 70) +
      (dbReview.summary.length > 70 ? '...' : '')
  ...
  const csScore = isValidNumber(dbReview.cs_score)
    ? Math.max(0, Math.min(100, dbReview.cs_score))
    : 70;
  ...
  const images = (() => {
    if (
      dbReview.car_models?.image_url &&
      isValidUrl(dbReview.car_models.image_url)
    ) {
  ...
  return {
    id: isValidNumber(dbReview.id)
      ? dbReview.id.toString()
      : `review-${Date.now()}`,
    ...
    car_models: dbReview.car_models
      ? transformDatabaseCarModelToCarModel(
          dbReview.car_models as DatabaseCarModel,
        )
      : undefined,
  };
}

export function transformDatabaseCarModelToCarModel(
  dbModel: DatabaseCarModel,
): CarModel {
  if (!dbModel || typeof dbModel !== 'object') {
    logger.warn('Invalid database car model provided to transformer:', dbModel);
    return {
      /* default/empty CarModel structure */
    } as CarModel;
  }
  ...
  return {
    ...
    description: isValidString(dbModel.description)
      ? dbModel.description.trim()
      : undefined,
    image_url:
      isValidString(dbModel.image_url) && isValidUrl(dbModel.image_url)
        ? dbModel.image_url
    ...
    brands: dbModel.brands
      ? {
          ...
          name: isValidString(dbModel.brands.name)
            ? dbModel.brands.name.trim()
            : 'Unknown Brand',
          logo_url:
            isValidString(dbModel.brands.logo_url) &&
            isValidUrl(dbModel.brands.logo_url)
              ? dbModel.brands.logo_url
              : undefined,
        }
      : undefined,
    is_active: dbModel.is_active !== undefined ? dbModel.is_active : true,
    created_at: isValidDate(dbModel.created_at)
      ? dbModel.created_at
      : undefined,
    updated_at: isValidDate(dbModel.updated_at)
      ? dbModel.updated_at
      : undefined,
  };
}
...
export function transformDatabaseBrandToBrand(dbBrand: DatabaseBrand): Brand {
  ...
  return {
    ...
    logo_url:
      isValidString(dbBrand.logo_url) && isValidUrl(dbBrand.logo_url)
        ? dbBrand.logo_url
        : undefined,
    description: isValidString(dbBrand.description)
      ? dbBrand.description.trim()
      : undefined,
    ...
    created_at: isValidDate(dbBrand.created_at)
      ? dbBrand.created_at
      : undefined,
    updated_at: isValidDate(dbBrand.updated_at)
      ? dbBrand.updated_at
      : undefined,
  };
}
