...
export async function fetchVehicleListings(
  ...
  try {
    ...
    logger.debug('🔍 Fetching vehicle listings from Supabase...', {
      page,
      limit,
      searchQuery,
    });
    ...
    logger.debug(
      '✅ Successfully fetched vehicle listings:',
      data?.length || 0,
    );
    const result = data || [];
...
export async function fetchCarReviews(
  ...
  try {
    ...
    const cacheKey = getCacheKey('carReviews', [
      page,
      limit,
      searchQuery,
      sortBy,
    ...
    logger.debug('🔍 Fetching car reviews from Supabase...', {
      page,
      limit,
      searchQuery,
      sortBy,
...
export async function fetchRecommendedCars() {
  try {
    ...
    logger.debug(
      '✅ Successfully fetched recommended cars:',
      data?.length || 0,
    );
    const result = data || [];
    setCache(cacheKey, result, 10 * 60 * 1000); // Cache for 10 minutes
...
export async function searchVehiclesWithFilters(searchParams: {
  ...
  try {
    ...
    if (
      searchParams.year &&
      (searchParams.year < 1900 ||
        searchParams.year > new Date().getFullYear() + 1)
    ) {
...
export async function fetchReviewById(
  ...
  if (!id || isNaN(parseInt(id))) {
    // Assuming review ID is a number
  ...
  try {
    ...
    if (!data) {
      logger.warn(
        `⚠️ No data returned for review ID ${id}, though no explicit error.`,
      );
      return null;
    }
...
export async function fetchVehicleListingById(
  id: string,
): Promise<DatabaseVehicleListing | null> {
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        // Resource not found
        logger.warn(
          `⚠️ Vehicle listing with ID ${id} not found or not active.`,
        );
    ...
    if (!data) {
      logger.warn(
        `⚠️ No data returned for vehicle listing ID ${id}, though no explicit error.`,
      );
      return null;
    }
...
export async function addBookmark(
  targetType: 'listing' | 'review' | 'model',
  targetId: string,
) {
  try {
...
export async function checkIfCarSaved(
  userId: string,
