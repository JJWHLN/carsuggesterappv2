...
interface SearchStore extends SearchState {
  ...
  updateSavedSearch: (
    id: string,
    updates: Partial<SearchState['savedSearches'][0]>,
  ) => void;
...
const useSearchStore = create<SearchStore>()(
  persist(
    subscribeWithSelector((set, get) => ({
      ...
      // Saved Searches Actions
      saveSearch: (name, query, filters, alertsEnabled = false) =>
        set((state) => {
          const savedSearch = {
            id: Date.now().toString(),
            name,
      ...
      // Filter Presets Actions
      saveFilterPreset: (name, filters, isDefault = false) =>
        set((state) => {
          const preset = {
            id: Date.now().toString(),
            name,
      ...
      setActiveFilterCategory: (category) =>
        set({ activeFilterCategory: category }),

      // Quick Filters Actions
      addQuickFilter: (id, label, filters) =>
        set((state) => ({
          quickFilters: [
            ...state.quickFilters,
            { id, label, filters, isActive: false },
          ],
        })),
      ...
      toggleQuickFilter: (id) =>
        set((state) => {
          ...
          const activeQuickFilters = updatedQuickFilters.filter(
            (f) => f.isActive,
          );
          const combinedFilters = activeQuickFilters.reduce(
            (acc, filter) => ({
              ...acc,
              ...filter.filters,
            }),
      ...
      addToRecentSearches: (query) =>
        set((state) => {
          ...
          return {
            recentSearches: [query, ...filtered].slice(
              0,
