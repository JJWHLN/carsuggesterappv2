...
interface UserStore {
  ...
  applyForDealerAccount: (
    dealerInfo: UserProfile['dealerInfo'],
  ) => Promise<void>;
...
const useUserStore = create<UserStore>()(
  persist(
    subscribeWithSelector((set, get) => ({
      ...
      setOnboardingStep: (step) =>
        set((state) => ({
          profile: state.profile
            ? { ...state.profile, onboardingStep: step }
            : null,
        })),
      ...
      // Activity Tracking
      trackCarView: (carId, duration, source) =>
        set((state) => ({
          activity: {
            ...state.activity,
            carViews: [
      ...
      getUserStats: () => {
        ...
        return {
          ...
          averageSessionTime:
            activity.carViews.reduce((acc, view) => acc + view.duration, 0) /
              activity.carViews.length || 0,
        };
      },
