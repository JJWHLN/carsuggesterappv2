...
export const MultiSelectFilter: React.FC<MultiSelectFilterProps> = React.memo(
  ({
    ...
    const handleSelectAll = useCallback(() => {
      ...
      const hasAllSelected = allValues.every((value) =>
      ...
      ({ item }: { item: FilterOption }) => {
        ...
        return (
          ...
          >
            ...
            {item.icon && <Text className="mr-2 text-lg">{item.icon}</Text>}
            ...
            <View className="flex-1">
              ...
              {item.count !== undefined && (
                <Text className="text-xs text-gray-500">{item.count} cars</Text>
              )}
            </View>
          </TouchableOpacity>
        );
    ...
    const renderVirtualizedOption = useCallback(
      (info: { item: FilterOption; index: number }) => {
    ...
    const getItem = useCallback(
      (data: FilterOption[], index: number) => data[index],
      [],
    );
