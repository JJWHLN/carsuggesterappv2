...
export const PriceRangeSlider: React.FC<PriceRangeSliderProps> = React.memo(({
  ...
  const minPosition = useSharedValue(((value[0] - min) / (max - min)) * SLIDER_WIDTH);
  const maxPosition = useSharedValue(((value[1] - min) / (max - min)) * SLIDER_WIDTH);
  ...
  const snapToStep = useCallback((position: number): number => {
    const rawValue = min + (position / SLIDER_WIDTH) * (max - min);
    const steppedValue = Math.round(rawValue / step) * step;
    return Math.max(min, Math.min(max, steppedValue));
  }, [min, max, step]);

  const onValueChange = useCallback((newMin: number, newMax: number) => {
    const sortedValues: [number, number] = newMin <= newMax ? [newMin, newMax] : [newMax, newMin];
  ...
  const minPanResponder = PanResponder.create({
    ...
    onPanResponderMove: (_, gestureState) => {
      const newPosition = Math.max(0, Math.min(SLIDER_WIDTH, minPosition.value + gestureState.dx));
  ...
  const maxPanResponder = PanResponder.create({
    ...
    onPanResponderMove: (_, gestureState) => {
      const newPosition = Math.max(0, Math.min(SLIDER_WIDTH, maxPosition.value + gestureState.dx));
