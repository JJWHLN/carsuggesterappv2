...
import { MarketInsights as MarketInsightsType, SimilarCarPricing } from './types';
  ...
  return (
    <View className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
      <View className="flex-row items-center justify-between mb-3">
        <View className={`w-12 h-12 ${color} rounded-lg items-center justify-center`}>
...
const SimilarCarCard: React.FC<SimilarCarCardProps> = ({ car, basePrice }) => {
  ...
  return (
    <View className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm mr-4" style={{ width: 280 }}>
      ...
      <View className="flex-row justify-between items-center mb-3">
        ...
        <View className="flex-row items-center">
          ...
          <Text className={`font-medium ${isHigher ? 'text-red-600' : 'text-green-600'}`}>
            {isHigher ? '+' : ''}{formatCurrency(priceDiff)}
          </Text>
        </View>
      </View>
...
export const MarketInsights: React.FC<MarketInsightsProps> = ({ insights, similarCars }) => {
    ...
    <ScrollView className="flex-1 bg-gray-50">
          ...
          {marketAnalysis && (
            <View className="bg-white rounded-lg p-4 border border-gray-200 mb-6">
              ...
              <View className={`p-4 rounded-lg mb-4 ${
                marketAnalysis.marketCondition.type === 'buyers' ? 'bg-green-50 border border-green-200' :
                marketAnalysis.marketCondition.type === 'sellers' ? 'bg-red-50 border border-red-200' :
                'bg-yellow-50 border border-yellow-200'
              }`}>
                <View className="flex-row items-center mb-2">
                  <Users size={20} className={
                    marketAnalysis.marketCondition.type === 'buyers' ? 'text-green-600' :
                    marketAnalysis.marketCondition.type === 'sellers' ? 'text-red-600' :
                    'text-yellow-600'
                  } />
                  <Text className={`ml-2 font-bold text-lg ${
                    marketAnalysis.marketCondition.type === 'buyers' ? 'text-green-900' :
                    marketAnalysis.marketCondition.type === 'sellers' ? 'text-red-900' :
                    'text-yellow-900'
                  }`}>
                    {marketAnalysis.marketCondition.type === 'buyers' ? "Buyer's Market" :
                     marketAnalysis.marketCondition.type === 'sellers' ? "Seller's Market" :
                     "Balanced Market"}
                  </Text>
                </View>
                <Text className={`text-sm ${
                  marketAnalysis.marketCondition.type === 'buyers' ? 'text-green-800' :
                  marketAnalysis.marketCondition.type === 'sellers' ? 'text-red-800' :
                  'text-yellow-800'
                }`}>
                  Strength: {Math.round(marketAnalysis.marketCondition.strength * 100)}%
                </Text>
              </View>
      ...
      {activeTab === 'trends' && (
        <View className="p-4">
          ...
          {/* Supply Trend Chart */}
          <View className="bg-white rounded-lg p-4 border border-gray-200 mb-6">
            <Text className="font-semibold text-gray-900 mb-4">Inventory Levels</Text>
            <LineChart
          ...
          {/* Demand Chart */}
          <View className="bg-white rounded-lg p-4 border border-gray-200 mb-6">
            <Text className="font-semibold text-gray-900 mb-4">Demand Indicators</Text>
            <BarChart
              data={demandData}
              width={width - 64}
              height={200}
          ...
          <View className="bg-white rounded-lg p-4 border border-gray-200">
            <Text className="font-semibold text-gray-900 mb-4">Seasonal Patterns</Text>
            ...
            <View className="space-y-3">
              ...
              <View className="flex-row justify-between items-center">
                ...
                <Text className={`font-semibold ${
                  insights.seasonality.currentSeasonImpact < 0 ? 'text-green-600' : 'text-red-600'
                }`}>
      ...
      {activeTab === 'regional' && (
        <View className="p-4">
          ...
          {/* Regional Score */}
          <View className="bg-white rounded-lg p-4 border border-gray-200 mb-6">
            <Text className="font-semibold text-gray-900 mb-4">Market Score</Text>
            ...
            <View className="items-center mb-4">
              <View className="w-32 h-32 rounded-full border-8 border-gray-200 items-center justify-center relative">
                ...
                <View 
                  ...
                  style={{
                    transform: [{ rotate: `${insights.regional.regionScore * 360}deg` }],
                    borderTopColor: 'transparent',
                    borderRightColor: 'transparent',
                    borderBottomColor: insights.regional.regionScore > 0.5 ? undefined : 'transparent'
                  }}
                />
              </View>
            </View>
          ...
          {/* Price Variation */}
          <View className="bg-white rounded-lg p-4 border border-gray-200 mb-6">
            <Text className="font-semibold text-gray-900 mb-4">Price Variation</Text>
