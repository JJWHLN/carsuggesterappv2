...
const SmartCarComparison: React.FC<SmartCarComparisonProps> = ({
  ...
  const generateAdvancedComparison = useCallback(async () => {
    ...
    try {
      ...
      if (comparisonMode === 'ai_powered') {
        const aiComparison =
          await recommendationEngine.generateIntelligentComparison(
            carIds,
            userId,
            {
  ...
  const generateBasicComparison = useCallback(() => {
    const scores: ComparisonScore[] = cars.map((car) => {
      ...
      selectedCategories.forEach((category) => {
        const categoryMetrics = comparisonMetrics.filter(
  ...
  const getCarMetricValue = useCallback(
    (car: Car, metricId: string): number => {
      // Mock data - in production, this would come from car specifications
      const mockData: Record<string, Record<string, number>> = {
        [car.id]: {
          horsepower:
            car.make === 'BMW' ? 300 : car.make === 'Tesla' ? 350 : 250,
          acceleration:
            car.make === 'Tesla' ? 4.2 : car.make === 'BMW' ? 5.8 : 7.2,
          top_speed:
            car.make === 'Tesla' ? 250 : car.make === 'BMW' ? 280 : 200,
          price: car.price,
          fuel_economy:
          ...
          resale_value:
            car.make === 'Tesla' ? 0.75 : car.make === 'BMW' ? 0.65 : 0.55,
          tech_score:
            car.make === 'Tesla' ? 9.5 : car.make === 'BMW' ? 8.5 : 7.0,
          comfort_score:
            car.make === 'Mercedes-Benz' ? 9.0 : car.make === 'BMW' ? 8.5 : 7.5,
          cargo_space: car.make === 'Tesla' ? 425 : 400,
          safety_rating:
            car.make === 'Tesla' ? 5.0 : car.make === 'BMW' ? 4.8 : 4.5,
          adas_features:
            car.make === 'Tesla' ? 15 : car.make === 'BMW' ? 12 : 8,
          co2_emissions:
            car.fuel_type === 'Electric'
              ? 0
  ...
  const generatePros = useCallback(
    (car: Car, categoryScores: Record<string, number>): string[] => {
      const pros: string[] = [];
      ...
      if (categoryScores.environmental > 0.8)
        pros.push('Environmentally friendly');
  ...
  const generateCons = useCallback(
    (car: Car, categoryScores: Record<string, number>): string[] => {
      const cons: string[] = [];

      if (categoryScores.performance < 0.4) cons.push('Limited performance');
      if (categoryScores.economics < 0.4)
        cons.push('Higher than average costs');
  ...
  const determineRecommendation = useCallback(
    (
      ...
      totalScore: number,
      categoryScores: Record<string, number>,
    ): ComparisonScore['recommendation'] => {
  ...
  const formatMetricValue = useCallback(
    (value: number, format: ComparisonMetric['format']) => {
      switch (format) {
        case 'currency':
    ...
    return (
      ...
      >
        <View style={styles.emptyState}>
          ...
          >
            <Text
              style={[styles.addCarButtonText, { color: colors.background }]}
            >
              Add Cars to Compare
            </Text>
          </TouchableOpacity>
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text
                style={[styles.sectionToggle, { color: colors.textSecondary }]}
              >
          ...
          {comparisonData?.ai_insights && showAIInsights && (
            ...
            >
              ...
              <View style={styles.aiInsights}>
                ...
                >
                  <Text
                    style={[styles.insightTitle, { color: colors.primary }]}
                  >
                    🏆 Best Overall
                  </Text>
                ...
                {comparisonMetrics
                  ...
                  .map((metric) => (
                    ...
                    >
                      ...
                      {cars.map((car) => {
                        const value = getCarMetricValue(car, metric.id);
                        const normalizedScore = normalizeMetricValue(
                          value,
                          metric,
                          cars,
                        );
                        const isHighest = cars.every(
                          (otherCar) =>
                          ...
                          <View key={car.id} style={styles.carColumn}>
                            <Text
                              style={[
                                ...
                                {
                                  color:
                                    isHighest && metric.higherIsBetter
                                      ? colors.primary
                                      : colors.text,
                                  fontWeight:
                                    isHighest && metric.higherIsBetter
