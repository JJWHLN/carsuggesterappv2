  ...
  const renderComparisonRow = (item: ComparisonItem) => {
    ...
    return (
      <View key={item.key} style={styles.comparisonRow}>
        ...
        {cars.map((car, index) => (
          <View
            ...
            style={[
              ...
              {
                backgroundColor:
                  bestIndex === index ? colors.primaryLight : 'transparent',
          ...
          >
            ...
            {bestIndex === index && <Award color={colors.success} size={14} />}
          </View>
        ))}
        ...
        {cars.length < 3 && (
          <View
            style={[styles.rowValue, { borderRightColor: colors.border }]}
          />
        )}
      </View>
    );
      ...
      <Card style={styles.winnerCard}>
        ...
        <View style={styles.winnerContent}>
          ...
          <Text style={[styles.winnerScore, { color: colors.textSecondary }]}>
            Wins {winner.score} out of{' '}
            {comparisonItems.filter((item) => item.highlightBest).length}{' '}
            categories
          </Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={styles.headerRow}>
              <View style={styles.labelColumn}>
                <Text
                  style={[styles.columnLabel, { color: colors.textSecondary }]}
                >
                  Comparison
                </Text>
              </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={styles.comparisonTable}>
              {renderSection(
                'Overview',
                'overview',
                comparisonItems.slice(0, 4),
              )}
              {renderSection(
                'Financial',
                'financial',
                comparisonItems.slice(4, 5),
              )}
              {renderSection(
                'Performance',
                'performance',
                comparisonItems.slice(5, 8),
              )}
            </View>
          </ScrollView>
