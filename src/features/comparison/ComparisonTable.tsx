...
export const ComparisonTable: React.FC<ComparisonTableProps> = ({ className = "" }) => {
  ...
  const getFieldHighlight = (car: ExtendedCar, field: ComparisonField) => {
    ...
    if (field.highlightBest && currentNumeric === min && field.key.includes('acceleration')) return 'best'; // Lower is better for 0-60
    ...
    return comparisonSections
      ...
      .map(section => ({
        ...
        fields: section.fields.filter(field => 
          !state.showDifferencesOnly || hasRowDifferences(field)
        ),
      }))
                            ...
                            return (
                              <View
                                ...
                                className={`w-64 p-4 border-l border-gray-200 justify-center ${
                                  highlight === 'best' ? 'bg-green-50' : 
                                  highlight === 'worst' ? 'bg-red-50' : 
                                  hasRowDifferences(field) ? 'bg-yellow-50' : ''
                                }`}
                              >
                                <View className="flex-row items-center">
                                  {field.type === 'rating' && (
                                  ...
                                  <Text className={`text-sm ${
                                    highlight === 'best' ? 'text-green-700 font-semibold' :
                                    highlight === 'worst' ? 'text-red-700 font-semibold' :
                                    'text-gray-800'
                                  }`}>
                                  ...
                                  {highlight === 'best' && (
                                    <Award size={14} className="text-green-600 ml-1" />
                                  )}
                                </View>
                              </View>
                            );
