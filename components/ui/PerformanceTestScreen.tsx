...
export const PerformanceTestScreen: React.FC = () => {
  ...
  const { startTiming, endTiming, getMetrics, generateReport } =
    usePerformanceMonitor();
  ...
  const runPerformanceTests = async () => {
    ...
    try {
      ...
      results.push({
        test: 'App Load Time',
        status:
          loadTime < 3500 ? 'passed' : loadTime < 5000 ? 'warning' : 'failed',
        value: loadTime,
        threshold: 3500,
        unit: 'ms',
      ...
      results.push({
        test: 'Bundle Size',
        status:
          bundleSize < 40 ? 'passed' : bundleSize < 50 ? 'warning' : 'failed',
        value: bundleSize,
        threshold: 40,
        unit: 'MB',
      ...
      results.push({
        ...
        status:
          renderTime < 100 ? 'passed' : renderTime < 200 ? 'warning' : 'failed',
        value: renderTime,
        threshold: 100,
        unit: 'ms',
      ...
      results.push({
        ...
        status:
          apiTime < 500 ? 'passed' : apiTime < 1000 ? 'warning' : 'failed',
      ...
      results.push({
        ...
        status:
          iconCount < 80 ? 'passed' : iconCount < 120 ? 'warning' : 'failed',
        value: iconCount,
        threshold: 80,
        unit: 'icons',
      ...
      {testResults.map((result, index) => (
        <Card key={index} style={styles.resultCard}>
          ...
          <View style={styles.thresholdContainer}>
            ...
            <Text
              style={[styles.status, { color: getStatusColor(result.status) }]}
            >
