  ...
  ({
    ...
    useEffect(() => {
      if (
        mode === 'debounced' &&
        debouncedValue &&
        debouncedValue.length >= minCharacters
      ) {
    ...
    const handleClear = useCallback(() => {
      ...
      setSearchCount(0);
      if (onClear) onClear();
      trackMetric('search_cleared', value.length, undefined, {
        previous_length: value.length,
      });
    ...
    const handleSubmit = useCallback(() => {
      ...
      trackMetric('search_submitted', value.length, undefined, {
    ...
    const SuggestionModal = useMemo(() => {
      if (!showSuggestionModal || (!suggestions.length && !showSuggestions))
        return null;
      ...
      return (
        ...
        >
          ...
          >
            <Animated.View
              style={[styles.suggestionContainer, suggestionModalStyle]}
            >
              <FlatList
                data={suggestions}
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    <View style={styles.suggestionContent}>
                      ...
                      {item.type === 'popular' && (
                        <TrendingUp size={16} color={currentColors.primary} />
                      )}
                      {item.type === 'brand' && (
                        <Star size={16} color={currentColors.primary} />
                      )}
      ...
      const performSearch = async () => {
        try {
          ...
          setSearchHistory((prev) => {
            const updated = [
              debouncedQuery,
              ...prev.filter((item) => item !== debouncedQuery),
            ];
