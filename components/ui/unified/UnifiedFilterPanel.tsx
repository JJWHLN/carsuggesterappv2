...
import { currentColors, BorderRadius, Spacing, Typography, Shadows } from '@/constants/Colors';
...
type FilterConfig = SingleSelectFilter | MultiSelectFilter | RangeFilterConfig | ToggleFilter | TextFilter;
...
export const UnifiedFilterPanel: React.FC<UnifiedFilterPanelProps> = React.memo(({
  ...
  // Animations
  const slideAnim = useRef(new Animated.Value(variant === 'bottom-sheet' ? height : 0)).current;
  ...
  const loadMoreFilters = useCallback(() => {
    if (enableLazyLoading && visibleFilters.length < filters.length) {
      const nextBatch = filters.slice(visibleFilters.length, visibleFilters.length + maxInitialFilters);
      setVisibleFilters(prev => [...prev, ...nextBatch]);
      trackMetric('filters_loaded', nextBatch.length);
    }
  }, [enableLazyLoading, visibleFilters.length, filters, maxInitialFilters, trackMetric]);
  ...
  const toggleSection = useCallback((sectionId: string) => {
    setExpandedSections(prev => {
      ...
      trackMetric('filter_section_toggled', 1, undefined, { section_id: sectionId, expanded: !prev.has(sectionId) });
      return newSet;
    });
  }, [trackMetric]);

  const handleValueChange = useCallback((filterId: string, value: any) => {
    const newValues = { ...localValues, [filterId]: value };
    setLocalValues(newValues);
    onValuesChange(newValues);
    trackMetric('filter_changed', 1, undefined, { filter_id: filterId, filter_type: filters.find(f => f.id === filterId)?.type });
  ...
  const handleApply = useCallback(() => {
    ...
    if (onClose) onClose();
    trackMetric('filters_applied', 1, undefined, { filter_count: Object.keys(localValues).length });
  ...
  const handleSavePreset = useCallback(() => {
    if (presetName.trim() && onSavePreset) {
      ...
      trackMetric('preset_saved', 1, undefined, { preset_name: presetName.trim() });
    }
  ...
  const renderFilter = useCallback((filter: FilterConfig) => {
    ...
    const FilterHeader = (
      ...
      >
        <Text style={[styles.filterTitle, { color: colors.text }]}>{filter.title}</Text>
        {isExpanded ? (
    ...
    const FilterContent = () => {
      switch (filter.type) {
            ...
            <View style={styles.optionsContainer}>
              {filter.options.map((option) => (
                ...
                >
                  ...
                  {option.count && (
                    <Text style={[styles.optionCount, { color: colors.textMuted }]}>
                      ({option.count})
                    </Text>
                  )}
        ...
        case 'range':
          const [min, max] = currentValue || [filter.range.min, filter.range.max];
          return (
            <View style={styles.rangeContainer}>
              ...
              {filter.range.unit && (
                <Text style={[styles.rangeUnit, { color: colors.textMuted }]}>
                  {filter.range.unit}
                </Text>
              )}
            </View>
          ...
          return (
            <View style={styles.toggleContainer}>
              <Switch
                ...
                thumbColor={currentValue ? colors.primary : colors.neutral400}
              />
            </View>
          );
        ...
        case 'text':
          return (
            <TextInput
              ...
              placeholder={filter.placeholder || `Enter ${filter.title.toLowerCase()}`}
  ...
  const PresetSection = useMemo(() => {
    ...
    return (
      <View style={styles.presetSection}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>Saved Filters</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          ...
          {onSavePreset && (
            <TouchableOpacity
              style={[styles.presetChip, styles.savePresetChip, { borderColor: colors.border }]}
        ...
        {showPresetInput && (
          <View style={styles.presetInputContainer}>
            ...
            />
            <TouchableOpacity
              style={[styles.savePresetButton, { backgroundColor: colors.primary }]}
              onPress={handleSavePreset}
            >
              <Text style={[styles.savePresetButtonText, { color: colors.white }]}>Save</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
    );
  }, [showPresets, presets, colors, handleLoadPreset, onSavePreset, showPresetInput, presetName, handleSavePreset]);
  ...
  }, [localValues, colors, showClearButton, showApplyButton, handleClear, handleApply]);
    ...
    <View style={[styles.container, style]}>
      ...
      <ScrollView 
        ...
        onScroll={({ nativeEvent }) => {
          ...
          if (enableLazyLoading && nativeEvent.contentOffset.y > nativeEvent.contentSize.height - nativeEvent.layoutMeasurement.height - 100) {
            loadMoreFilters();
          }
        }}
