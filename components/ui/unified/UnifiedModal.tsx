...
import { X, AlertTriangle, CheckCircle, Info } from '@/utils/ultra-optimized-icons';
...
// Types
type ModalVariant = 'default' | 'fullscreen' | 'bottom-sheet' | 'center' | 'alert' | 'confirmation';
...
export const UnifiedModal: React.FC<UnifiedModalProps> = React.memo(({
  ...
  useEffect(() => {
    if (visible) {
      ...
      if (showTime.current) {
        ...
        trackMetric('modal_usage', duration, {
          good: 5000,
  ...
  }, [visible, animationType, variant, opacity, scale, translateY, onShow, onDismiss, trackMetric]);
  ...
  const ActionsComponent = useMemo(() => {
    ...
    if (primaryAction) allActions.push({ ...primaryAction, style: 'primary' });
    if (secondaryAction) allActions.push({ ...secondaryAction, style: 'secondary' });
      ...
      <View style={styles.actions}>
        {allActions.map((action, index) => (
          <TouchableOpacity
            ...
            style={[
              styles.actionButton,
              action.style === 'primary' && [styles.primaryButton, { backgroundColor: colors.primary }],
              action.style === 'destructive' && [styles.destructiveButton, { backgroundColor: colors.error }],
              action.style === 'secondary' && [styles.secondaryButton, { borderColor: colors.border }],
              action.disabled && styles.disabledButton,
            ]}
            onPress={() => {
              trackMetric('modal_action', 1, undefined, { action_type: action.style || 'default' });
              action.onPress();
            }}
          ...
          >
            <Text
              style={[
                ...
                action.style === 'primary' && [styles.primaryButtonText, { color: colors.white }],
                action.style === 'destructive' && [styles.destructiveButtonText, { color: colors.white }],
                action.style === 'secondary' && [styles.secondaryButtonText, { color: colors.text }],
                action.disabled && [styles.disabledButtonText, { color: colors.textMuted }],
              ]}
            >
              {action.label}
            </Text>
  ...
  const MainContent = (
    <Animated.View
      style={[
        ...
        (variant === 'center' || variant === 'alert' || variant === 'confirmation') && styles.centerContainer,
        containerStyle,
        style,
      ]}
      testID={testID}
      ...
      case 'fullscreen':
        return (
          <SafeAreaView style={styles.fullscreenWrapper}>
            <KeyboardWrapper style={styles.fullscreenInner} {...keyboardProps}>
              {MainContent}
            </KeyboardWrapper>
          </SafeAreaView>
        );
      ...
      case 'bottom-sheet':
        return (
          <View style={styles.bottomSheetWrapper}>
            ...
            <KeyboardWrapper style={styles.bottomSheetInner} {...keyboardProps}>
              {MainContent}
            </KeyboardWrapper>
          </View>
        );
