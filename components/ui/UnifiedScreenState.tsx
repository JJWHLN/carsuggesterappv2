...
export const UnifiedScreenState = memo<UnifiedScreenStateProps>(({
  ...
  const isDataEmpty = showEmptyWhenNoData && (
    isEmpty ||
  ...
  if (loading && !showLoadingOverlay) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background }, style, loadingStyle]}>
  ...
  if (error && !loading) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background }, style, errorStyle]}>
        <ErrorState
          title={errorTitle}
          message={error}
          onRetry={onRetry}
  ...
  if (isDataEmpty && !loading && !error) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background }, style, emptyStyle]}>
        <EmptyState
          title={emptyTitle}
...
export const LoadingScreenState = memo<Pick<UnifiedScreenStateProps, 'loadingText' | 'loadingSize' | 'style'>>(
...
export const ErrorScreenState = memo<Pick<UnifiedScreenStateProps, 'error' | 'errorTitle' | 'onRetry' | 'style'>>(
...
export const EmptyScreenState = memo<Pick<UnifiedScreenStateProps, 'emptyTitle' | 'emptySubtitle' | 'emptyIcon' | 'emptyAction' | 'style'>>(
...
export function useScreenState<T>(
  ...
  const isEmpty = showEmptyWhenNoData && (
    (Array.isArray(data) && data.length === 0) ||
    (!Array.isArray(data) && !data)
  );
  
  const renderScreenState = (children: React.ReactNode, props: Partial<UnifiedScreenStateProps> = {}) => (
    <UnifiedScreenState
      loading={loading}
      error={error}
      isEmpty={isEmpty}
