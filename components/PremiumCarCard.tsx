...
import { MapPin, Heart, Star, Eye, Zap, Clock, TrendingUp, Award } from '@/utils/ultra-optimized-icons';
...
const PremiumCarCard = memo<PremiumCarCardProps>(({ 
  ...
  const premiumFeatures = {
    isElectric: car.make?.toLowerCase().includes('tesla') || Math.random() > 0.7,
  ...
  const PremiumBadge = ({ type, text }: { type: string; text: string }) => {
    const badgeStyle = type === 'Electric' ? styles.badgeElectric :
                      type === 'Certified' ? styles.badgeCertified :
                      type === 'Premium' ? styles.badgePremium : styles.badge;
    
    const textStyle = type === 'Electric' ? styles.badgeElectricText :
                     type === 'Certified' ? styles.badgeCertifiedText :
                     type === 'Premium' ? styles.badgePremiumText : styles.badgeText;
  ...
  const StatusIndicator = ({ icon, count, color }: { icon: React.ReactNode; count: number; color: string }) => (
    ...
    >
      ...
      >
        ...
        <View style={styles.imageContainer}>
          <OptimizedImage
            source={{ uri: (car as any).image_url || 'https://via.placeholder.com/400x300' }}
            style={styles.image}
            resizeMode="cover"
          />
            ...
            <View style={styles.actionRow}>
              ...
              >
                <Animated.View style={{ transform: [{ scale: likeAnim }] }}>
                  <BlurView intensity={80} style={[styles.blurButton, isLiked && styles.likedButton]}>
                    <Heart 
          ...
          <View style={styles.bottomRow}>
            ...
            <View style={styles.priceContainer}>
              ...
              {(car as any).original_price && (car as any).original_price > car.price && (
...
const getThemedStyles = (colors: any, variant: string) => {
  const baseCardHeight = variant === 'compact' ? 200 : variant === 'featured' ? 320 : 280;
