...
import { MapPin, Star, CheckCircle, Clock, User, Award, TrendingUp, Users, Calendar, Eye } from '@/utils/ultra-optimized-icons';
...
export const DealershipReviewCard = memo<DealershipReviewCardProps>(({
  ...
  const getReviewTypeDisplay = (type: string) => {
    switch (type) {
      case 'expert_visit': return 'Expert Visit Review';
      case 'mystery_shopper': return 'Mystery Shopper Review';
      case 'customer_experience': return 'Customer Experience Review';
      default: return 'Dealership Review';
    }
  };

  const renderRatingBreakdown = () => {
    const ratings = [
      { label: 'Customer Service', value: review.customer_service_rating, icon: Users },
      { label: 'Sales Process', value: review.sales_process_rating, icon: TrendingUp },
      { label: 'Facility', value: review.facility_rating, icon: Award },
      { label: 'Pricing', value: review.pricing_transparency_rating, icon: Shield },
          ...
          <View key={index} style={styles.ratingRow}>
            ...
            <View style={styles.ratingStars}>
              {[1, 2, 3, 4, 5].map((star) => (
                <Star
                  key={star}
                  color={star <= rating.value ? colors.warning : colors.border}
                  size={12}
  ...
  const renderDealershipScore = () => {
    ...
    return (
      <View style={styles.dealershipScore}>
        ...
        <Text style={[styles.scoreText, { color: getScoreColor(review.dealership_score) }]}>
          ...
          <View style={styles.headlineContent}>
            ...
            <View style={styles.headlineMain}>
              {showDealerInfo && review.dealers && (
                <View style={styles.dealerInfo}>
                  <Text style={styles.dealerName}>{review.dealers.business_name}</Text>
            ...
            <View style={styles.headlineBottom}>
              <View style={styles.overallRating}>
                <Star color={colors.warning} size={20} fill={colors.warning} />
      ...
      {showDealerInfo && review.dealers && (
        <View style={styles.dealerInfoCard}>
          <View style={styles.dealerHeader}>
            <Text style={styles.dealerNameCard}>{review.dealers.business_name}</Text>
            {review.dealers.verified && (
              <CheckCircle color={colors.success} size={14} fill={colors.success} />
            )}
          </View>
      ...
      <View style={styles.reviewMeta}>
        <View style={styles.authorInfo}>
          ...
          <View style={styles.authorDetails}>
            ...
            <Text style={styles.authorCredentials}>{review.author_credentials}</Text>
          </View>
        </View>
