...
import { MapPin, Heart, Star, Clock, Fuel } from '@/utils/ultra-optimized-icons';
  ...
  const renderGridView = () => (
    <Card style={StyleSheet.flatten([styles.card, style])}>
      <View style={styles.imageContainer}>
        ...
        {showSaveButton && (
          <TouchableOpacity
            style={[styles.saveButton, { backgroundColor: isBookmarked ? colors.primary : 'rgba(0,0,0,0.6)' }]}
            onPress={handleBookmarkToggle}
            disabled={bookmarkLoading}
            accessibilityLabel={isBookmarked ? 'Remove from saved cars' : 'Save car'}
          >
            <Heart 
              size={16} 
      ...
      <View style={styles.listContent}>
        ...
        {car.dealer && (
          <View style={styles.listDealerRow}>
            <Text style={[styles.listDealer, { color: colors.textSecondary }]}>
              {car.dealer.name}
            </Text>
            {car.dealer.verified && (
              <View style={styles.listVerifiedBadge}>
                <Star size={10} color={colors.primary} />
                <Text style={[styles.listVerifiedText, { color: colors.primary }]}>
                  Verified
                </Text>
              </View>
            )}
...
const getStyles = (colors: any, variant: 'grid' | 'list') => StyleSheet.create({
  // Grid view styles
  card: {
    overflow: 'hidden',
    marginBottom: 16,
