...
import { smartSearchService, SearchSuggestion, SearchQuery } from '@/services/SmartSearchService';
...
export const SmartSearchBar: React.FC<SmartSearchBarProps> = ({
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      const suggestions = await smartSearchService.getSearchSuggestions(query, maxSuggestions);
  ...
  const executeSearch = useCallback(async (searchQuery: string, additionalFilters?: any) => {
    ...
    try {
      ...
      if (userId && showRecommendations) {
        const recommendations = await aiRecommendationEngine.getPersonalizedRecommendations(userId, 4);
  ...
  const showSuggestions = useCallback(() => {
    ...
    suggestionsHeight.value = withSpring(Math.min(300, maxSuggestions * 60 + 20));
  }, [maxSuggestions]);
  ...
  const getSuggestionIcon = (type: SearchSuggestion['type']): keyof typeof Ionicons.glyphMap => {
    switch (type) {
      case 'make': return 'car-outline';
      case 'model': return 'car-sport-outline';
  ...
  return (
    <View style={[styles.container, style]}>
        ...
        <Animated.View style={[
          ...
          {state.suggestions.length > 0 ? (
            <FlatList
              ...
              keyExtractor={(item, index) => `suggestion-${item.value}-${index}`}
      ...
      {showRecommendations && state.recommendations.length > 0 && !state.showSuggestions && (
        <View style={[styles.recommendationsContainer, { backgroundColor: colors.surface }]}>
            ...
            renderItem={({ item }) => (
              <RecommendationCard
                ...
                onPress={() => {
                  ...
                  executeSearch(`${item.car.make} ${item.car.model} ${item.car.year}`);
                }}
              />
            )}
...
const RecommendationCard: React.FC<RecommendationCardProps> = ({
  ...
  const getCategoryColor = () => {
    switch (category) {
      case 'perfect_match': return colors.success;
      case 'good_fit': return colors.primary;
