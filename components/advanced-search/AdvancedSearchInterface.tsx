...
> = ({
  ...
  const generateMLSuggestions = useCallback(
    async (query: string): Promise<SearchSuggestion[]> => {
      ...
      if (
        ...
        query.toLowerCase().includes('kids')
      ) {
        suggestions.push({
          id: 'nl-2',
          type: 'natural_language',
      ...
      if (
        query.toLowerCase().includes('dublin') ||
        query.toLowerCase().includes('cork')
      ) {
        suggestions.push({
          id: 'location-1',
          type: 'location',
  ...
  const getContextualSuggestions = useCallback((): SearchSuggestion[] => {
    ...
    if (showTrendingSearches) {
      const trending = ['Tesla Model 3', 'Hybrid SUVs', 'Luxury sedans 2024'];
  ...
  useEffect(() => {
    if (
      enableMLSuggestions &&
      suggestions.some((s) => s.type === 'natural_language')
    ) {
      Animated.loop(
        Animated.sequence([
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        <View style={styles.inputContainer}>
          ...
          {enableMLSuggestions && (
            <Animated.View
              style={[styles.mlIndicator, { opacity: sparkleAnimation }]}
            >
