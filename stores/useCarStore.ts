  ...
  subscribeWithSelector((set, get) => ({
    ...
    loadFeaturedCars: async () => {
      ...
      try {
        ...
        set({ 
          error: error instanceof Error ? error.message : 'Failed to load featured cars',
          isLoadingFeatured: false 
        });
      }
    },
    ...
    loadRecommendedCars: async (userId) => {
      ...
      try {
        // TODO: Replace with actual API call
        const url = userId ? `/api/cars/recommended?userId=${userId}` : '/api/cars/recommended';
        const response = await fetch(url);
        const cars = await response.json();
        set({ recommendedCars: cars, isLoadingRecommended: false });
      } catch (error) {
        set({ 
          error: error instanceof Error ? error.message : 'Failed to load recommended cars',
          isLoadingRecommended: false 
        });
      }
    },
    ...
    // Recently Viewed
    addToRecentlyViewed: (car) => set((state) => {
      const filtered = state.recentlyViewed.filter(c => c.id !== car.id);
      return {
        recentlyViewed: [{ ...car, lastViewed: new Date().toISOString() }, ...filtered].slice(0, 20)
      };
    }),
    ...
    // Bookmarks
    toggleBookmark: (carId) => set((state) => {
    ...
    removeFromComparison: (carId) => set((state) => ({
