...
interface UIStore {
  ...
  showAlert: (title: string, message: string, type?: 'info' | 'warning' | 'error' | 'success') => string;
  ...
  subscribeWithSelector((set, get) => ({
    ...
    hideModal: (id) => set((state) => {
      ...
      return {
        ...
        activeModal: updatedModals.length > 0 ? updatedModals[updatedModals.length - 1].id : null
      };
    }),
    ...
    // Navigation
    setCurrentRoute: (route, params = {}) => set((state) => {
    ...
    toggleGridView: () => set((state) => ({
      uiPreferences: { ...state.uiPreferences, gridView: !state.uiPreferences.gridView }
    })),
    
    toggleCompactMode: () => set((state) => ({
      uiPreferences: { ...state.uiPreferences, compactMode: !state.uiPreferences.compactMode }
    })),
    ...
    toggleComparisonDrawer: () => set((state) => ({ comparisonDrawerOpen: !state.comparisonDrawerOpen })),
    ...
    setFormTouched: (formId, field, touched) => set((state) => {
      ...
      return {
        formTouched: {
          ...state.formTouched,
          [formId]: { ...currentForm, [field]: touched } as Record<string, boolean>
        }
      };
    }),
