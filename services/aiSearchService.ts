...
export class AISearchService {
  ...
  async search(
    ...
    try {
      ...
      const filters = intent ? openaiService.convertIntentToFilters(intent) : {};
      logger.debug('Generated filters:', filters);

      // Step 3: Build and execute Supabase query
      const { results, totalCount } = await this.executeSupabaseSearch(filters, page, limit);
      logger.debug(`Found ${totalCount} total results, returning ${results.length}`);

      // Step 4: Generate explanation and suggestions
      const explanation = await openaiService.explainSearchResults(query, totalCount, intent || undefined);
      ...
      return {
        ...
        suggestions: ['Try a more specific search', 'Browse by category', 'Filter by price range'],
        processingTimeMs: processingTime,
        confidence: 0.3
      };
    }
  ...
  private async executeSupabaseSearch(
    ...
    if (filters.features && filters.features.length > 0) {
      ...
      const featureQueries = filters.features.map(feature => `features.cs.{${feature}}`).join(',');
  ...
  private async basicSearch(
    ...
    try {
      ...
      const searchQuery = supabase
        ...
        `)
        .eq('status', 'active')
        .or(`make.ilike.%${query}%,model.ilike.%${query}%,title.ilike.%${query}%`);
  ...
  async getPopularSearches(limit: number = 5): Promise<string[]> {
    try {
      ...
      const uniqueQueries = [...new Set(data?.map(item => item.original_query) || [])];
