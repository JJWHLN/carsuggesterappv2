...
export default function AddCarScreen() {
  ...
  const validateSingleField = (fieldId: string, value: any) => {
    ...
    const rules =
      VehicleValidationRules[fieldId as keyof typeof VehicleValidationRules];
    if (!rules) return '';
  ...
  const handleSubmit = async () => {
    ...
    const validationFields: ValidationFormField[] = formFields
      .filter(
        (field) =>
          VehicleValidationRules[
            field.id as keyof typeof VehicleValidationRules
          ],
      )
      .map((field) => ({
        name: field.id,
  ...
  const FormField = ({ field }: { field: FormField }) => {
    ...
    return (
      <Card style={styles.fieldCard}>
        ...
        {showError && <Text style={styles.errorText}>{fieldError}</Text>}
      </Card>
    );
  };
      ...
      >
          ...
          <View style={styles.photoGrid}>
            {selectedImages.map((image, index) => (
              <View key={index} style={styles.photoContainer}>
                <OptimizedImage source={{ uri: image }} style={styles.photo} />
                <TouchableOpacity
          ...
          <Card style={styles.fieldCard}>
            ...
            <TouchableOpacity style={[styles.fieldInput, styles.textAreaInput]}>
              <Text style={[styles.fieldInputText, styles.placeholderText]}>
                Describe your car's condition, features, maintenance history,
                etc.
              </Text>
            </TouchableOpacity>
          </Card>
        ...
        <View style={styles.submitSection}>
          <Button
            ...
            variant="primary"
            icon={
              uploading ? undefined : <Upload color={colors.white} size={20} />
            }
            style={
              uploading
          ...
          <Text style={styles.submitNote}>
            By submitting, you agree to our terms and conditions. Your listing
