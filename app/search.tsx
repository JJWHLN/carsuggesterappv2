...
const SearchScreen = memo(() => {
  ...
  const [sortBy, setSortBy] = useState<
    'relevance' | 'price_low' | 'price_high' | 'year_new' | 'year_old'
  >('relevance');
  ...
  const fetchData = useCallback(
    async (page: number, limit: number) => {
      ...
      const dbCars = await fetchVehicleListings(
        page,
        limit,
        sanitizedQuery || undefined,
      );
      return Array.isArray(dbCars)
        ? dbCars.map(transformDatabaseVehicleListingToCar)
        : [];
    },
  ...
  const handleAISearch = useCallback(async () => {
    ...
    try {
      ...
      const transformedCars = Array.isArray(dbCars)
        ? dbCars.map(transformDatabaseVehicleListingToCar)
        : [];
  ...
  const renderCar: ListRenderItem<Car> = useCallback(
    ({ item }) => (
  ...
  const renderFooter = useCallback(() => {
    ...
    return (
      <View style={styles.footerLoader}>
        ...
        <Text style={[styles.loadingText, { marginLeft: Spacing.sm }]}>
          Loading more cars...
        </Text>
      </View>
    );
  ...
  const renderExampleChip = useCallback(
                ...
                {bodyTypes.map((type) => (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setFilters((prev) => ({
                        ...prev,
                        bodyType: prev.bodyType === type ? '' : type,
                      }))
                    }
                  >
                    <Text
                ...
                {fuelTypes.map((fuel) => (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setFilters((prev) => ({
                        ...prev,
                        fuelType: prev.fuelType === fuel ? '' : fuel,
                      }))
                    }
                  >
                    <Text
      ...
      <SafeAreaView style={styles.container}>
        <FlatList<Car>
          ...
          contentContainerStyle={
            displayCars.length === 0 ? styles.emptyContent : styles.content
          }
