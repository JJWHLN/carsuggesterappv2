...
export interface UseAdvancedAnalyticsReturn {
  ...
  trackCarInteraction: (action: string, carId: string, properties?: Record<string, any>) => void;
  trackSearchBehavior: (action: string, properties?: Record<string, any>) => void;
  trackRecommendationInteraction: (action: string, properties?: Record<string, any>) => void;
...
export const useAdvancedAnalytics = (): UseAdvancedAnalyticsReturn => {
  ...
  const trackCarInteraction = useCallback((action: string, carId: string, properties: Record<string, any> = {}) => {
  ...
  const trackSearchBehavior = useCallback((action: string, properties: Record<string, any> = {}) => {
  ...
  const trackRecommendationInteraction = useCallback((action: string, properties: Record<string, any> = {}) => {
  ...
  const generateRecommendations = useCallback(async (userId: string): Promise<string[]> => {
    try {
      const recommendations = await analyticsService.generatePersonalizedRecommendations(userId);
      ...
      trackError('recommendation_generation_failed', 'useAdvancedAnalytics', { user_id: userId });
      return [];
    }
